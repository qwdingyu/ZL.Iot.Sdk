...
namespace ZL.DataConvert
{
    class Object
    {
        ...
        /// <summary>
        /// 指定的字符串转换成指定长度的 object数组
        /// </summary>
        /// <param name="strValue">字符串</param>
        /// <param name="objLength">长度</param>
        /// <returns></returns>
        //原方法名：GetObjValue
        //第2个参数指转换后的数组长度。TEST-->[T,E,S,T,0,0]
        public static object[] Str2Chars(string strValue, int objLength)
        {
            var objValue = new object[objLength];
            for (var i = 0; i < strValue.Length; i++)
            {
                objValue[i] = strValue.ToCharArray().GetValue(i);
            }
            return objValue;
        }
