...
namespace ZL.DB.Acc
{
    /// <summary>
    /// 旧式仓储兼容层。
    /// 推荐新项目优先使用 <see cref="Repository{T}"/> 或直接注入 <see cref="SqlSugarScope"/>。
    /// 本类型保留用于兼容历史业务代码，避免一次性迁移带来的风险。
    ///
    /// 使用方式：
    /// public class SysDeptRepository : BaseRepository<Sys_dept>
    /// {
    ///     public SysDeptRepository(SqlSugarClient dbContext) : base(dbContext)
    ///     {
    ///     }
    /// }
    /// </summary>
    ...
    public class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        public bool Update(T entity, List<string> list = null, bool isNull = true)
        {
            if (list == null)
            {
                list = new List<string>() { "CreateBy", "CreateDate", "created_by", "created_at" };
            }
        ...
        {
            //https://github.com/kevin-montrose/Jil/blob/master/Jil/JSON.cs
