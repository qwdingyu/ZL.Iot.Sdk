...
namespace ZL.DataConvert
{
    ...
    public class ByteArrayLib
    {
        ...
        /// <summary>
        /// 返回指定长度的byte[]数组，如果len小于strVal.length，转换后byte[]数组长度为strVal.length
        /// </summary>
        /// <param name="strVal">字符串</param>
        /// <param name="len">需要返回的数组的长度</param>
        /// <returns></returns>
        //第2个参数指转换后的字符串长度。
        //string转byte[]类型.//TEST-->[84,69,83,84，0]，同上
        public static byte[] strs2Asc(string strVal, int len)
        {
            var asciiEncoding = new ASCIIEncoding();
            var tmp = asciiEncoding.GetBytes(strVal.PadRight(len, char.MinValue));
            return tmp;
        }
